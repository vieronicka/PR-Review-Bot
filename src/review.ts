...
/** Markdown body for GitHub PR review (Phase 3) */
export function formatReviewBody(review: ReviewResult): string {
  ...
  const footer = process.env.GITHUB_ACTIONS === "true"
    ? "_Posted by GitHub Actions (pr-review-agent workflow)._"
    : "_Generated by pr-review-agent CLI — summary review._";
...
export function printReview(
  review: ReviewResult,
  options?: { posted?: boolean; reviewUrl?: string },
): void {
  console.log("\n--- AI review ---\n");
  console.log("Summary");
  console.log("-------");
  ...
  if (review.lineComments && review.lineComments.length > 0) {
    console.log("Line comments (terminal only — not posted as inline GitHub comments)");
    console.log("------------------------------------------------------------------");
  ...
  if (options?.posted && options.reviewUrl) {
    console.log(`Posted to GitHub: ${options.reviewUrl}\n`);
  } else if (options?.posted) {
    console.log("Posted to GitHub.\n");
  } else {
    console.log(
      "Review complete (terminal only). Add --post to publish on the PR.\n",
    );
  }
}
